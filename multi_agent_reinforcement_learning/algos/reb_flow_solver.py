...
def solveRebFlow(
    env: AMoD, res_path: str, CPLEXPATH: str, model_data_pairs: T.List[ModelDataPair]
):
    """Solves the Reb Flow."""
    t = env.time
    ...
    for model_data_pair in model_data_pairs:
        ...
        with open(datafile, "w") as file:
            ...
            file.write(
                "acc_init_tuple="
                + mat2str(model_data_pair.actor_data.cplex_data.acc_init_tuple)
                + ";\r\n"
            )
            file.write(
                "acc_actor_tuple="
                + mat2str(model_data_pair.actor_data.cplex_data.acc_actor_tuple)
                + ";\r\n"
            )
        ...
        model_data_pair.actor_data.actions.reb_action = [
            flow[i, j] for i, j in env.edges
        ]
